    ...
    <AnimatePresence mode="wait">
      <TabsPrimitive.Content asChild forceMount={forceMount} value={value}>
        <motion.div
          ...
          // Local change from the upstream component, which mounts at
          // opacity 0 and fades in. Motion's frame loop is paused while the
          // document is hidden, so a panel mounted in a background tab stays
          // fully transparent until the loop resumes -- the panel is present,
          // sized and focusable, but invisible. Panel content must never be
          // gated behind an animation completing, so it mounts at its settled
          // state; the exit fade and the animated tab highlight still carry
          // the transition.
          initial={false}
