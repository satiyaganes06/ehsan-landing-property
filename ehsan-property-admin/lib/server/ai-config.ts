...
/* Read straight from process.env rather than the API's zod env module, which
   threw at import time. A missing key must surface as "AI is not configured"
   on the one screen that uses it, not as a build failure for the whole app. */
const env = {
  AI_PROVIDER: (process.env.AI_PROVIDER ?? 'none') as 'openrouter' | 'anthropic' | 'none',
  OPENROUTER_API_KEY: process.env.OPENROUTER_API_KEY ?? '',
  OPENROUTER_MODEL: process.env.OPENROUTER_MODEL ?? 'google/gemma-4-31b-it:free',
