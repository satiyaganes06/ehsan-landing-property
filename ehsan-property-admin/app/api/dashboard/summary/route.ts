...
export const GET = route({ resource: 'project', action: 'read' }, async () => {
  const [
    ...
    prisma.seoMeta.findMany({ where: { score: { lt: 50 } }, orderBy: { score: 'asc' }, take: 8 }),
  ...
  const lowScoring = await Promise.all(
    seoBad.map(async (s) => {
      const title =
  ...
  // 30-day enquiry trend, bucketed by day in JS: Prisma's groupBy cannot
  // bucket by day portably, and 30 rows is cheap to reduce here.
  const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  const forTrend = await prisma.enquiry.findMany({
    where: { createdAt: { gte: since } },
    select: { createdAt: true },
  });
  const trend = new Map<string, number>();
  for (const e of forTrend) {
    const day = e.createdAt.toISOString().slice(0, 10);
    trend.set(day, (trend.get(day) ?? 0) + 1);
  }

  return json({
    needsAttention: { lowScoring, mediaMissingAlt: missingAlt },
    publishState: {
      draftProjects, draftEvents,
      scheduledCount: scheduledProjects + scheduledEvents,
    ...
    searchConsole: { enabled: false, reason: 'Not configured.' },
  });
});
